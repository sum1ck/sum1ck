<div align="center">
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=33&duration=2500&pause=800&color=5B3EC9&center=true&vCenter=true&width=700&lines=sumick;Python+Developer;FastAPI+%2F+Django+%2F+React" />
</div>

---

## Stack

<div align="center">
<img src="https://skillicons.dev/icons?i=python,fastapi,django,js,react,postgres,redis,docker,git,linux&perline=6" />
</div>

---

## What I actually do well

```txt
> Design REST APIs with FastAPI/Django that are clean and easy to hand off.
> Build async task pipelines with Celery + Redis that remove manual bottlenecks.
> Take systems from first commit to a working, deployed product.
> Working with AI as much as possible: use Cursor, Windsurf and Claude Code daily to move faster
  without cutting corners on code quality.
```

---

## Current focus

```txt
Backend architecture
Async processing
API design
React
```
